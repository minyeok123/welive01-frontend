...
import { fetchResidentNoticeDetail, postNoticeView } from '@/entities/notice/api/noticeApi';
...
export default function DetailResidentNoticePage() {
  ...
  useEffect(() => {
    ...
    const viewKey = `welive_notice_view_${noticeId}`;
    ...
    const fetchData = async () => {
      try {
        ...
        if (shouldCountView) {
          try {
            const { viewsCount } = await postNoticeView(noticeId);
            setNotice((prev) => (prev ? { ...prev, viewsCount } : prev));
          } catch {
