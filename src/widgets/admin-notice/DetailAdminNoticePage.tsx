...
import { postNoticeView } from '@/entities/notice/api/noticeApi';
...
export default function DetailAdminNoticePage() {
  ...
  useEffect(() => {
    ...
    const viewKey = `welive_notice_view_${idStr}`;
    ...
    const fetchData = async () => {
      try {
        const response = await axiosInstance.get(`/notices/${idStr}`);
        setData(response.data);
        if (shouldCountView) {
          try {
            const { viewsCount } = await postNoticeView(idStr);
            setData((prev) => (prev ? { ...prev, viewsCount } : prev));
          } catch {
