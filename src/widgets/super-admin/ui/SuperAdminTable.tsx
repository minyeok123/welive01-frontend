...
export default function SuperAdminTable({ data, currentPage, totalCount }: Props) {
  ...
  const handleApprove = async (registerId: string) => {
    if (!registerId) {
      alert('가입 신청 정보를 찾을 수 없습니다. 목록을 새로고침해 주세요.');
      return;
    }
    try {
      await updateAdminApprovalStatus(registerId, ApprovalStatus.APPROVED);
      alert('승인 완료');
  ...
  const handleReject = async (registerId: string) => {
    if (!registerId) {
      alert('가입 신청 정보를 찾을 수 없습니다. 목록을 새로고침해 주세요.');
      return;
    }
    try {
      await updateAdminApprovalStatus(registerId, ApprovalStatus.REJECTED);
      alert('거절 완료');
      window.location.reload();
    } catch (error) {
      console.error(error);
                    ...
                    <ul className='flex items-center justify-center gap-4'>
                      <li>
                        <button
                          ...
                          onClick={() => handleApprove(item.adminRegisterId ?? '')}
                        >
                          승인
                        </button>
                      </li>
                      <li>
                        <button
                          ...
                          onClick={() => handleReject(item.adminRegisterId ?? '')}
                        >
                          거절
                        </button>
                      </li>
