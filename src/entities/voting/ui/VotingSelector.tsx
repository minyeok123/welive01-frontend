...
export default function VotingSelector({
  ...
  const pollDongCode = buildingPermission.toString().padStart(4, '0');
  const residentDongCode =
    residentDong != null && residentDong !== ''
      ? Number(residentDong).toString().padStart(4, '0')
      : '';
  // 백엔드 POST /options/:id/vote 는 userAuthorize(입주민 USER만) — 관리자는 API가 403
  const canVote =
    role === 'USER' &&
    (buildingPermission === 0 || (residentDongCode !== '' && pollDongCode === residentDongCode));
  ...
  const voteButtonLable = (() => {
    if (role !== 'USER') return '입주민만 투표할 수 있습니다';
